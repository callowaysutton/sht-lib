...
TEST(BoostCoverage, FormatStrNull) {
    char buffer[64];
    sht_format_str(buffer, sizeof(buffer), NULL, 0);
}

TEST(BoostCoverage, FormatStrTruncate) {
    char buffer[128];
    char long_str[100];
    memset(long_str, 'A', 99);
    long_str[99] = '\0';
    sht_format_str(buffer, sizeof(buffer), long_str, 1);
}
